...
class PwGaANLayer(nn.Module):
    def __init__(self, in_dim, out_dim, num_heads=1, att=True, gate=True, use_pre_w=True):
        ...
        if self.att:
            ...
            self.Wa = nn.Linear(self.in_dim, self.out_dim, bias=False) #nn.linear Applies a linear transformation
            print(self.Wa)
            # to the incoming data: y = xA^T + b
    ...
    def reset_parameters(self):
        ...
        gain = nn.init.calculate_gain('leaky_relu')  #returns root 2 for relu
        if self.att:
            nn.init.xavier_normal_(self.Wa.weight, gain=gain)
            nn.init.xavier_normal_(self.att_out_fc_l.weight, gain=gain)
            #Fills the input Tensor with values a=gain× root 6/fan_in+fan_out
    ...
    def reduce_func(self, nodes):
        ...
        if self.att:
            ...
            # We perform a degree bucketing mechanism where messages for nodes with a same degree are processed together. Let’s say the
            # three dimensions are separately (A, B, C), then A is the number of nodes with the same degree, B is the
            # number of messages received for each node (in most cases this is equivalent to degree), and C is the size
            # of message features.

            alpha = F.dropout(alpha, 0.1)
            #During training, randomly zeroes some of the
            # elements of the input tensor with probability p=0.1 using samples from a Bernoulli distribution.
